...
/* The name of a directory to precede the filename when producing. */
char *production_directory = (char *)NULL;
  ...
  while (arg_index < argc && argv[arg_index][0] == '-')
      ...
      else if (strcmp (arg, "-d") == 0)
	{
	  ...
	  if (production_directory)
	    free (production_directory);

	  production_directory = xmalloc (2 + strlen (argv[arg_index]));
	  strcpy (production_directory, argv[arg_index]);
	  len = strlen (production_directory);

	  if (len && production_directory[len - 1] != '/')
	    strcat (production_directory, "/");
     ...
     char *arg;
{
  ...
  if (defs->production)
	...
	return (0);
	  ...
	  sprintf(path, "%s%s", production_directory ? production_directory : "", defs->production);

      defs->output = fopen (path, "w");
