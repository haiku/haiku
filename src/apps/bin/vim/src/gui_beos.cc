...
/*
 * If you don't have R3 or later,
 * switch this to 0.
 */
#define HAVE_R3_OR_LATER 1

/*
 * Turn on to use inline IM input.
 */
//#ifdef FEAT_MBYTE_IME
# define USE_INLINE_IM
//#endif
...
VimTextAreaView::MessageReceived(BMessage *m)
{
    ...
    case B_INPUT_METHOD_EVENT:
	{
	    ...
	    case B_INPUT_METHOD_CHANGED:
		{
			...
			while (i < str.Length())
			{
			    clen = utf_ptr2len_check(chars+i);
...
VimTextAreaView::mchDrawString(int row, int col, char_u *s, int len, int flags)
{
	...
	while((p - (char*)s) < len) {
#ifdef FEAT_MBYTE
	    clen = utf_ptr2len_check((u_char*)p);
#else
	    clen = 1;
#endif
...
void VimTextAreaView::DrawIMString(void)
{
	...
	clen = utf_ptr2len_check((char_u *)(s+i));
...
static char appsig[] = "application/x-vnd.Rhialto-Vim";
key_map *keyMap;
char *keyMapChars;
...
#ifdef FEAT_MBYTE
    int cells = utf_off2cells(LineOffset[gui.row] + gui.col);
    if(cells>=4) cells = 1;
