...
//        unsigned int nLength = *((unsigned int *) &cID3v2Header[6]);
	...
	nBytesRead = 1;
	int	nScanBytes = 0;
	while ( (nGoalID != nReadID) && (nBytesRead > 0) && (nScanBytes < (1024*1024)) ) {
		unsigned char	cTemp[1024];
		m_pIO->Read(&cTemp, sizeof(cTemp), &nBytesRead);
		for ( unsigned int i = 0 ; i < sizeof(cTemp) ; i++ ) {
			nReadID = (((unsigned int)cTemp[i]) << 24) | (nReadID >> 8);
			nJunkBytes++;
			if ( nGoalID == nReadID )
				break;
		}
		nScanBytes += sizeof(cTemp);
	}
	// <-- SHINTA
