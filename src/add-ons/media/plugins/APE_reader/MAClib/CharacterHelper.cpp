...
//        pANSI[nCharacters] = 0;
		memcpy(pANSI, pUTF16, nCharacters + 1);
    #endif
...
str_utf16 * GetUTF16FromUTF8(const str_utf8 * pUTF8)
{
	...
	str_utf16 * pUTF16 = new str_utf16 [nCharacters + 1];
	memcpy(pUTF16, pUTF8, nCharacters + 1);
    return pUTF16; 
}
...
str_utf8 * GetUTF8FromUTF16(const str_utf16 * pUTF16)
{
    ...
    int nUTF8Index = 0;
    for (int z = 0; z < nCharacters; z++)	// SHINTA
    {
        if (pUTF16[z] < 0x0080)
        {
    ...
    str_utf8 * pUTF8 = new str_utf8 [nCharacters + 1];
	memcpy(pUTF8, pUTF16, nCharacters + 1);
