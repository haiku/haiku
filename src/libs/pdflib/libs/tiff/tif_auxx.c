/* PDFlib GmbH cvsid: $Id: tif_auxx.c,v 1.1 2004/10/06 17:46:51 laplace Exp $ */
...
TIFFDefaultTransferFunction(TIFF* tif, TIFFDirectory* td)
{
	...
	tf[0] = (uint16 *)_TIFFmalloc(tif, n * sizeof (uint16));
	tf[0][0] = 0;
	for (i = 1; i < n; i++) {
	...
	if (td->td_samplesperpixel - td->td_extrasamples > 1) {
		tf[1] = (uint16 *)_TIFFmalloc(tif, n * sizeof (uint16));
		_TIFFmemcpy(tf[1], tf[0], n * sizeof (uint16));
		tf[2] = (uint16 *)_TIFFmalloc(tif, n * sizeof (uint16));
