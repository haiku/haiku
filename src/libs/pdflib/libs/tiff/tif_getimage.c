/* PDFlib GmbH cvsid: $Id: tif_getimage.c,v 1.1 2004/10/06 17:46:51 laplace Exp $ */
...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	(void) pickTileContigCase(tif, img);
    } else {
	img->get = TIFFIsTiled(tif) ? gtTileSeparate : gtStripSeparate;
	(void) pickTileSeparateCase(tif, img);
    }
    return (1);
}
...
initYCbCrConversion(TIFF* tif, TIFFRGBAImage* img)
{
    uint16 hs, vs;
...
pickTileContigCase(TIFF* tif, TIFFRGBAImage* img)
{
	...
	case PHOTOMETRIC_YCBCR:
	    if (img->bitspersample == 8)
		put = initYCbCrConversion(tif, img);
	    break;
	}
    }
