/* PDFlib GmbH cvsid: $Id: tif_dirread.c,v 1.1 2004/10/06 17:46:51 laplace Exp $ */
...
{
	char *cp = (char*)_TIFFmalloc(tif, n);
	if (cp == NULL)
...
TIFFFetchPerSampleShorts(TIFF* tif, TIFFDirEntry* dir, int* pl)
{
	...
	if (CheckDirCount(tif, dir, (uint32) samples)) {
		...
		if (samples > (int)NITEMS(buf))
			v=(uint16*) _TIFFmalloc(tif, samples * sizeof (uint16));
...
TIFFFetchPerSampleAnys(TIFF* tif, TIFFDirEntry* dir, double* pl)
{
	...
	if (CheckDirCount(tif, dir, (uint32) samples)) {
		...
		if (samples > (int)NITEMS(buf))
			v=(double*) _TIFFmalloc(tif, samples * sizeof (double));
...
TIFFFetchExtraSamples(TIFF* tif, TIFFDirEntry* dir)
{
	...
	if (dir->tdir_count > NITEMS(buf))
		v=(uint16*) _TIFFmalloc(tif, dir->tdir_count * sizeof (uint16));
	if (dir->tdir_type == TIFF_BYTE)
