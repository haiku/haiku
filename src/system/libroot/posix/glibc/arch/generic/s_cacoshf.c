...
__cacoshf (__complex__ float x)
{
  ...
  if (rcls <= FP_INFINITE || icls <= FP_INFINITE)
  ...
  else
    {
      ...
      float ro = sqrtf (sq * sq + 4 * re2 * im2);
      float a = sqrtf ((sq + ro) / 2.0);
      float b = sqrtf ((-sq + ro) / 2.0);

      __real__ res = 0.5 * logf (re2 + __real__ x * 2 * a
					   + im2 + __imag__ x * 2 * b
					   + ro);
      __imag__ res = atan2f (__imag__ x + b, __real__ x + a);
#endif
    }
