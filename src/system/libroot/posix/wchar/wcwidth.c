...
static int
is_legacy_cjk_encoding(const char* encoding)
{
	return
		...
		|| strcasecmp(encoding, "CP949") == 0
		|| strcasecmp(encoding, "JOHAB") == 0;
}
...
wcwidth(wchar_t uc)
{
	...
	if ((uc >> 9) < 240) {
		...
		if (ind >= 0)
			if ((nonspacing_table_data[64 * ind + ((uc >> 3) & 63)] >> (uc & 7))
				& 1) {
				if (uc > 0 && uc < 0xa0)
					return -1;
				else
	...
	if (uc >= 0x00A1 && uc < 0xFF61 && uc != 0x20A9) {
		...
		if (is_legacy_cjk_encoding(encoding))
			return 2;
	}
