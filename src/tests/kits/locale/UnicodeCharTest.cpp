...
class UnicodeCharTest : public CppUnit::TestFixture {
	CPPUNIT_TEST_SUITE(UnicodeCharTest);
	CPPUNIT_TEST(AsciiChar_UnicodeChar_ReturnsCorrectProperties);
	CPPUNIT_TEST(ISO8859Char_UnicodeChar_ReturnsCorrectProperties);
	CPPUNIT_TEST(UTF8Char_UnicodeChar_ReturnsCorrectProperties);
	...
	void _TestChar(uint32 i, Result result)
	{
		char text[16];

		_ToString(i, text);
		CPPUNIT_ASSERT_EQUAL(BString(result.value), BString(text));
...
public:
	void AsciiChar_UnicodeChar_ReturnsCorrectProperties()
	{
		Result results[] = {
	...
	void ISO8859Char_UnicodeChar_ReturnsCorrectProperties()
	{
	...
	void UTF8Char_UnicodeChar_ReturnsCorrectProperties()
	{
...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(UnicodeCharTest, getTestSuiteName());
