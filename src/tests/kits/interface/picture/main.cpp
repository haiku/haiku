...
bool HasAttribute(const XML_Char **attributes, const char *name) {
    while (*attributes && strcmpi(*attributes, name) != 0)
        attributes += 2;
...
rgb_color GetColorAttribute(const XML_Char **attributes, const char *name, uint8 alpha) {
	...
	for (int32 i = 0; colors[i].name != NULL; i++)
		if (strcmpi(colors[i].name, attr) == 0) {
...
void CheckAttributes(const XML_Char **attributes, BView *view) {
	...
	if (HasAttribute(attributes, "stroke-linecap")) {
		...
		if (strcmpi(stroke_linecap, "but") == 0)
			fState.fLineCap = B_BUTT_CAP;
		else if (strcmpi(stroke_linecap, "round") == 0)
			fState.fLineCap = B_ROUND_CAP;
		else if (strcmpi(stroke_linecap, "square") == 0)
	...
	if (HasAttribute(attributes, "stroke-linejoin")) {
		...
		if (strcmpi(stroke_linejoin, "miter") == 0)
			fState.fLineJoin = B_MITER_JOIN;
		else if (strcmpi(stroke_linejoin, "round") == 0)
			fState.fLineJoin = B_ROUND_JOIN;
		else if (strcmpi(stroke_linejoin, "bevel") == 0)
...
void startElement(BView *view, const XML_Char *name, const XML_Char **attributes) {
    ...
    else if (strcmpi(name, "polygon") == 0) {
        BShape shape;
    ...
    else if (strcmpi(name, "polyline") == 0) {
        BShape shape;
        GetPolygonAttribute(attributes, "points", shape);
        fState.fMatrix.Transform(shape);
...
void endElement(BView *view, const XML_Char *name){
    Pop(view);
}
...
int main() {
    BApplication *app = new BApplication("application/x-vnd.Be-SVG2");
    BRect frame = BRect(100, 50, 300, 400);
    Svg2PictureWindow *w = new Svg2PictureWindow(frame, "lion.svg");
    w->Show();
    app->Run();
