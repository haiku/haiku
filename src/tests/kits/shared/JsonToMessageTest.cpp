...
class JsonToMessageTest : public CppUnit::TestFixture {
	CPPUNIT_TEST_SUITE(JsonToMessageTest);
	CPPUNIT_TEST(TestHaikuDepotFetchBatch);
	CPPUNIT_TEST(TestUnterminatedArray);
	CPPUNIT_TEST(TestUnterminatedObject);
	CPPUNIT_TEST(TestObjectC);
	CPPUNIT_TEST(TestObjectB);
...
public:
	void TestHaikuDepotFetchBatch()
	{
		...
		status_t result = BJson::Parse((const char*)input, message);
	...
	void TestObjectB()
	{
		...
		CPPUNIT_ASSERT_EQUAL(0, (int)reinterpret_cast<addr_t>(ptrValue));
	}
	...
	void TestArrayA()
	{
		...
		CPPUNIT_ASSERT_EQUAL_MESSAGE("!eq [1]", 4567.0, doubleValue);
...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(JsonToMessageTest, getTestSuiteName());
