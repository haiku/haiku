...
class LRUCacheTest : public CppUnit::TestFixture {
	...
	CPPUNIT_TEST(TestAddWithOverflow);
	CPPUNIT_TEST(TestAddWithOverflowWithGets);
...
public:
	...
	*/
	...
	{
		BPrivate::LRUCache<HashString, BString> map(5);
	...
	*/
	...
	{
		BPrivate::LRUCache<HashString, BString> map(3);
...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(LRUCacheTest, getTestSuiteName());
