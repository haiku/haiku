...
bool btRPCSend(int session, bt_outPacket *outPacket)
{
	...
	do
	{
		int bytes = btSend(session, outPacket->buffer + bytesSent, outPacket->length - bytesSent, 0);
		if (bytes == -1)
			return false;

		bytesSent += bytes;
	} while (bytesSent < outPacket->length);
...
bool btRPCCheckSignature(int session)
{
	...
	if (btRecv(session, signature, sigLen, 0) == -1)
		return false;
...
bt_inPacket *btRPCSimpleCall(unsigned int serverIP, int port, bt_outPacket *outPacket)
{
	...
	if (FD_ISSET(session, &sockSet))
		...
		{
			if (btRecv(session, &xid, sizeof(int32), 0) == -1 ||
				btRecv(session, &length, sizeof(int32), 0) == -1)
				goto abortCall;
			...
			if (length > 0 && length < BT_RPC_MAX_PACKET_SIZE)
			{
				...
				if (buffer)
				{
					...
					do
					{
						int bytes = btRecv(session, buffer + bytesRead, length - bytesRead, 0);
						if (bytes == -1)
						{
							free(buffer);
							goto abortCall;
...
int btSend(int sock, void *data, int dataLen, int flags)
{
	...
	for (;;)
	{
		bytes = send(sock, (const char *) data, dataLen, flags);
		if (bytes == -1)
			if (errno == EINTR)
				continue;
			else
...
void btRPCPutString(bt_outPacket *packet, char *buffer, int length)
{
	if (packet && buffer)
	{
		...
		btRPCPutInt32(packet, B_HOST_TO_LENDIAN_INT32(length));
