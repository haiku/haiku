...
ClientConnection::Init()
{
	...
	fQueryHandles = new(nothrow) NodeHandleMap("query handles");
	if (!fQueryHandles)
		return B_NO_MEMORY;
	...
	fNodeMonitoringEvents = new(nothrow) NodeMonitoringEventQueue;
...
ClientConnection::UserRemoved(User* user)
{
	...
	volumes = new(nothrow) ClientVolume*[fVolumes->Size()];
	if (!volumes) {
		...
		if (concerned) {
			...
			if (unmountAll) {
				...
				UserSecurityContext* userSecurityContext
					= new(nothrow) UserSecurityContext;
...
ClientConnection::VisitMountRequest(MountRequest* request)
{
	...
	if (result == B_OK) {
		securityContext = new(nothrow) UserSecurityContext;
		if (securityContext) {
...
ClientConnection::VisitReadQueryRequest(ReadQueryRequest* request)
{
	...
	int32* volumeIDs = new(nothrow) int32[volumeCount];
	if (!volumeIDs)
		result = B_NO_MEMORY;
...
ClientConnection::_CreateVolume(ClientVolume** _volume)
{
	...
	ClientVolume* volume = new(nothrow) ClientVolume(fSecurityContextLock,
		this);
	if (!volume)
		return B_NO_MEMORY;
...
ClientConnection::_UnmountVolume(ClientVolume* volume)
{
	...
	if (VolumeUnmountedEvent* event = new(nothrow) VolumeUnmountedEvent) {
		VolumeManagerLocker managerLocker;
	...
	NodeMonitoringRequest*& _request)
{
	...
	EntryCreatedRequest* request = new(nothrow) EntryCreatedRequest;
	if (!request)
		return B_NO_MEMORY;
	ObjectDeleter<NodeMonitoringRequest> requestDeleter(request);
	...
	NodeMonitoringRequest*& _request)
{
	...
	// allocate the request
	EntryRemovedRequest* request = new(nothrow) EntryRemovedRequest;
	if (!request)
		return B_NO_MEMORY;
	ObjectDeleter<NodeMonitoringRequest> requestDeleter(request);
...
{
	// allocate the request
	EntryMovedRequest* request = new(nothrow) EntryMovedRequest;
	if (!request)
		return B_NO_MEMORY;
	ObjectDeleter<NodeMonitoringRequest> requestDeleter(request);
	...
	StatChangedEvent* event, NodeMonitoringRequest*& _request)
{
	...
	// allocate the request
	StatChangedRequest* request = new(nothrow) StatChangedRequest;
	if (!request)
		return B_NO_MEMORY;
	ObjectDeleter<NodeMonitoringRequest> requestDeleter(request);
	...
	AttributeChangedEvent* event, NodeMonitoringRequest*& _request)
{
	...
	// allocate the request
	AttributeChangedRequest* request = new(nothrow) AttributeChangedRequest;
	if (!request)
		return B_NO_MEMORY;
	ObjectDeleter<NodeMonitoringRequest> requestDeleter(request);
