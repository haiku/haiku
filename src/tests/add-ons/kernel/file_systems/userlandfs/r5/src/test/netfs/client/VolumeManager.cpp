...
public:
	...
	void Close()
	{
		...
		while (VolumeEvent* event = fEvents.GetFirst()) {
	...
	VolumeEvent* Pop()
	{
		...
		AutoLocker<Locker> _(fLock);
		if (VolumeEvent* event = fEvents.GetFirst()) {
			fEvents.Remove(event);
			return event;
		}
	...
	const char* parameters, int32 len, Volume** volume)
{
	...
	fQueryManager = new(nothrow) QueryManager(this);
	if (!fQueryManager)
		return B_NO_MEMORY;
	...
	// create volumes set
	fVolumes = new(nothrow) VolumeSet;
	if (!fVolumes)
		return B_NO_MEMORY;
	...
	fNodeIDs2Volumes = new(nothrow) NodeIDVolumeMap;
	if (!fNodeIDs2Volumes)
		return B_NO_MEMORY;
	...
	RootVolume* rootVolume = new(nothrow) RootVolume(this);
	if (!rootVolume)
		return B_NO_MEMORY;
