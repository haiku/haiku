...
VolumeManager::Init()
{
	...
	fEntryCreatedEvents = new(nothrow) EntryCreatedEventMap;
	if (!fEntryCreatedEvents)
		return B_NO_MEMORY;

	// entry removed event map
	fEntryRemovedEvents = new(nothrow) EntryRemovedEventMap;
	if (!fEntryRemovedEvents)
		return B_NO_MEMORY;

	// entry moved event map
	fEntryMovedEvents = new(nothrow) EntryMovedEventMap;
	if (!fEntryMovedEvents)
		return B_NO_MEMORY;

	// node stat changed event map
	fNodeStatChangedEvents = new(nothrow) NodeStatChangedEventMap;
	if (!fNodeStatChangedEvents)
		return B_NO_MEMORY;

	// node attribute changed event map
	fNodeAttributeChangedEvents = new(nothrow) NodeAttributeChangedEventMap;
	if (!fNodeAttributeChangedEvents)
		return B_NO_MEMORY;

	// create the node monitor
	fNodeMonitor = new(nothrow) NodeMonitor(this);
	if (!fNodeMonitor)
		return B_NO_MEMORY;
	...
	fClientVolumes = new(nothrow) ClientVolumeMap;
	if (!fClientVolumes)
		return B_NO_MEMORY;
...
VolumeManager::CreateDefault()
{
	...
	VolumeManager* manager = new(nothrow) VolumeManager;
	if (!manager)
		return B_NO_MEMORY;
...
VolumeManager::LoadNode(const struct stat& st, Node** _node)
{
	...
	if (!node) {
		...
		if (S_ISDIR(st.st_mode))
			node = new(nothrow) Directory(volume, st);
		else
			node = new(nothrow) Node(volume, st);
		if (!node)
			return B_NO_MEMORY;
...
//PRINT(("  creating and adding entry...\n"));
		// create the entry
		entry = new(nothrow) Entry(volume, directory, name, node);
		if (!entry)
			return B_NO_MEMORY;
...
PRINT(("VolumeManager::OpenQuery(%p, \"%s\", 0x%lx, %ld, %ld)\n", queryDomain, queryString, flags, remotePort, remoteToken));
	...
	// allocate the handle
	QueryHandle* queryHandle = new(nothrow) QueryHandle(remotePort,
		remoteToken);
	if (!handle)
		return B_NO_MEMORY;
	...
	if (liveQuery) {
		queryHandler = new(nothrow) QueryHandler(this, queryDomain,
			queryHandle);
		if (!queryHandler)
			return B_NO_MEMORY;
...
VolumeManager::_AddVolume(dev_t volumeID, Volume** _volume)
{
	...
	// create the volume
	Volume* volume = new(nothrow) Volume(volumeID);
	if (!volume)
...
VolumeManager::_QueryEntryMoved(EntryMovedEvent* event)
{
	...
	// allocate the events
	EntryRemovedEvent* removedEvent = new(nothrow) EntryRemovedEvent;
	EntryCreatedEvent* createdEvent = new(nothrow) EntryCreatedEvent;
	...
	EntryCreatedEvent** _event)
{
	...
	// create the event
	EntryCreatedEvent* event = new(nothrow) EntryCreatedEvent;
	if (!event)
		return B_NO_MEMORY;
	...
	EntryRemovedEvent** _event)
{
	...
	// create the event
	EntryRemovedEvent* event = new(nothrow) EntryRemovedEvent;
	if (!event)
		return B_NO_MEMORY;
